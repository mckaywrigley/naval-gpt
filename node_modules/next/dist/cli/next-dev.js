...
const nextDev = async (argv)=>{
    ...
    async function preflight() {
        ...
        if (dependencies["@next/font"] || devDependencies["@next/font"] && devDependencies["@next/font"] !== "workspace:*") {
            Log.warn("Your project has `@next/font` installed as a dependency, please use the built-in `next/font` instead. " + "The `@next/font` package will be removed in Next.js 14. " + "You can migrate by running `npx @next/codemod built-in-next-font .`. Read more: https://nextjs.org/docs/messages/built-in-next-font");
        }
    }
  ...
  ${chalk.bold.cyan(`${pkgManager === "npm" ? "npx create-next-app" : `${pkgManager} create next-app`} --example with-turbopack with-turbopack-app`)}\n  cd with-turbopack-app\n  ${pkgManager} run dev
    ...
    if (args["--turbo"]) {
        ...
        if (!process.env.__NEXT_DISABLE_MEMORY_WATCHER && !isChildProcess) {
            ...
            const isDebugging = process.execArgv.some((localArg)=>localArg.startsWith("--inspect"));
            const isDebuggingWithBrk = process.execArgv.some((localArg)=>localArg.startsWith("--inspect-brk"));
            const debugPort = (()=>{
                var ref;
                const debugPortStr = (ref = process.execArgv.find((localArg)=>localArg.startsWith("--inspect") || localArg.startsWith("--inspect-brk"))) == null ? void 0 : ref.split("=")[1];
            ...
            const setupFork = (env, newDir)=>{
                ...
                childProcess == null ? void 0 : (ref3 = childProcess.stdout) == null ? void 0 : ref3.on("data", (chunk)=>{
                    filterForkErrors(chunk, "stdout");
                });
                childProcess == null ? void 0 : (ref2 = childProcess.stderr) == null ? void 0 : ref2.on("data", (chunk)=>{
            ...
            const instrumentationFilePaths = !!((ref1 = config.experimental) == null ? void 0 : ref1.instrumentationHook) ? (0, _utils1).getPossibleInstrumentationHookFilenames(dir, config.pageExtensions) : [];
            const wp = new _watchpack.default({
                ignored: (entry)=>{
                    return !(entry.split("/").length <= watchedEntryLength) && !instrumentationFilePaths.includes(entry);
                }
            });
            wp.watch({
                directories: [
            ...
            wp.on("aggregated", async ()=>{
                ...
                if (instrumentationFile) {
                    ...
                    if (instrumentationFileLastHash && instrumentationFileHash !== instrumentationFileLastHash) {
                        ...
                        if (!instrumentationFileLastHash && previousItems.size !== 0) {
                            (0, Log).warn("An instrumentation file was added, restarting the server to apply changes.");
                            return setupFork();
                        }
                ...
                for (const key1 of knownFiles.keys()){
                    previousItems.add(key1);
                }
                if (hasPagesApp) {
                    return;
                }
                ...
                try {
                    var ref5;
                    const result = (0, _findPagesDir).findPagesDir(newFiles[0], !!((ref5 = config.experimental) == null ? void 0 : ref5.appDir));
                    hasPagesApp = Boolean(result.pagesDir || result.appDir);
                } catch (_) {}
